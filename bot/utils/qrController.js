...
// Чтение PNG или JPG в {width,height,data (RGBA Buffer)}
...
// Рисует заполненный прямоугольник с закруглёнными углами (RGBA) на буфере dst (изменяет dst)
...
// Главная функция: делает QR, ресайз логотипа билинейно, рисует белую рамку с радиусом и накладывает логотип
export async function generate(text, imageFilePath, opts = {}) {
    const scale = opts.scale || 8;
    const margin = opts.margin || 4;
    const ecLevel = opts.ecLevel || 'H';
    const centerRatio = opts.centerRatio || 0.27; // логотип % ширины QR
    const framePadding = opts.framePadding || Math.max(8, Math.round(scale)); // отступ рамки вокруг логотипа в px
    const frameRadius = opts.frameRadius || Math.round((centerRatio * 0.5) * 100) / 100 * 12; // примерное значение, можно передать
    const frameColor = opts.frameColor || [255,255,255,255]; // белая рамка
    ...
    // оставим место на рамку: внутренняя область логотипа
    ...
    // позиционируем рамку и логотип по центру
    ...
    // Радиус скругления (клиент может переопределить); ограничим r
    let r = opts.frameRadius;
    if (!r) {
        r = Math.round(Math.min(totalFrameW, totalFrameH) * 0.12); // 12% от размера рамки по умолчанию
    }
    r = Math.max(1, Math.min(Math.floor(Math.min(totalFrameW, totalFrameH) / 2), r));

    // Нарисуем белую рамку под логотип (заполняем белым прямоугольник с закруглениями)
    drawFilledRoundedRect(qrPng.data, qrW, qrH, ox, oy, totalFrameW, totalFrameH, r, frameColor);

    // Теперь накладываем логотип поверх рамки (центрируем внутри рамки)
    ...
    //fs.writeFileSync(pngOutPath, outBuf);
